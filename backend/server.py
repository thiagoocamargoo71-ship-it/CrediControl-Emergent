...
async def sb_one(table, cols='*', eq=None, neq=None):
...
async def sb_many(table, cols='*', eq=None, neq=None, in_=None, order=None):
...
async def sb_count(table, eq=None):
...
async def sb_insert(table, doc):
    """Insert row(s). Returns inserted data."""
    r = await supabase.table(table).insert(doc).execute()
    return r.data


async def sb_update(table, data, eq=None):
...
async def sb_delete(table, eq=None):
...
async def get_current_user(request: Request) -> dict:
    ...
    try:
        ...
        user = await sb_one('users', 'id, name, email, role, is_blocked, created_at', eq={'id': payload["sub"]})
        if not user:
            raise HTTPException(status_code=401, detail="Usuário não encontrado")
        if user.get("is_blocked", False):
            raise HTTPException(status_code=403, detail="Usuário bloqueado")
        return {"id": user["id"], "name": user["name"], "email": user["email"], "role": user["role"], "created_at": user.get("created_at")}
...
async def register(data: UserRegister, response: Response):
    ...
    rows = await sb_insert('users', {
        "name": data.name, "email": email, "password_hash": hashed,
...
async def login(data: UserLogin, response: Response, request: Request):
    ...
    if not user or not verify_password(data.password, user["password_hash"]):
        ...
        if attempts:
            await sb_update('login_attempts', {"count": attempts["count"] + 1, "lockout_until": lockout_time}, eq={'identifier': identifier})
        else:
            await sb_insert('login_attempts', {"identifier": identifier, "count": 1, "lockout_until": lockout_time})
    ...
    user_id = user["id"]
    set_auth_cookies(response, create_access_token(user_id, email, user["role"]), create_refresh_token(user_id))
...
async def create_customer(data: CustomerCreate, user: dict = Depends(require_user)):
    rows = await sb_insert('customers', {
        ...
        "email": data.email, "document": data.document, "address": data.address,
        "notes": data.notes, "is_referral": data.is_referral,
...
async def get_customer(customer_id: str, user: dict = Depends(require_user)):
    customer = await sb_one('customers', '*', eq={'id': customer_id, 'user_id': user["id"]})
    if not customer:
...
async def get_customer_status(customer_id: str, user: dict = Depends(require_user)):
    ...
    unpaid = await sb_many('installments', 'due_date, status', in_={'loan_id': loan_ids}, neq={'status': 'paid'})
    today = datetime.now(timezone.utc).date()
    total_pending = total_overdue = 0
    for inst in unpaid:
        if datetime.strptime(inst["due_date"], "%Y-%m-%d").date() < today:
            total_overdue += 1
        else:
            total_pending += 1
    ...
    for k, v in data.model_dump().items():
        if v is not None:
            if k == "is_referral" and v is False:
                update_data.update({"is_referral": False, "referral_name": None, "referral_phone": None})
            else:
                update_data[k] = v
...
async def create_loan(data: LoanCreate, user: dict = Depends(require_user)):
    customer = await sb_one('customers', 'id, name', eq={'id': data.customer_id, 'user_id': user["id"]})
    if not customer:
        raise HTTPException(status_code=404, detail="Cliente não encontrado")
    ...
    rows = await sb_insert('loans', {
        "user_id": user["id"], "customer_id": data.customer_id,
        "amount": data.amount, "interest_rate": data.interest_rate,
        "total_amount": round(total_amount, 2),
        "number_of_installments": data.number_of_installments,
        "start_date": data.start_date, "interval_days": data.interval_days,
        "created_at": datetime.now(timezone.utc).isoformat()
    })
    loan = rows[0]
    loan_id = loan["id"]
...
async def delete_loan(loan_id: str, user: dict = Depends(require_user)):
    if not await sb_one('loans', 'id', eq={'id': loan_id, 'user_id': user["id"]}):
        raise HTTPException(status_code=404, detail="Empréstimo não encontrado")
    # CASCADE via FK handles installments and payments
...
async def list_installments(loan_id: str, user: dict = Depends(require_user)):
    ...
    for inst in insts:
        ...
        if status == "pending" and due_date < today:
            ...
            updated_amount = inst["amount"] + (inst["amount"] * daily_rate * days_overdue)
            status = "overdue"
            await sb_update('installments', {"status": "overdue", "updated_amount": round(updated_amount, 2)}, eq={'id': inst["id"]})

        result.append({
            "id": inst["id"], "loan_id": inst["loan_id"], "number": inst["number"],
            "amount": inst["amount"], "updated_amount": round(updated_amount, 2),
            "due_date": inst["due_date"], "status": status,
            "paid_at": inst.get("paid_at"), "days_overdue": days_overdue
        })
    return result
...
async def pay_installment(installment_id: str, user: dict = Depends(require_user)):
    ...
    await sb_insert('payments', {"installment_id": installment_id, "amount_paid": amount_paid, "payment_date": now.isoformat()})
    await sb_update('installments', {"status": "paid", "paid_at": now.isoformat()}, eq={'id': installment_id})
...
async def list_all_installments(
    ...
    for inst in insts:
        ...
        result.append({
            "id": inst["id"], "loan_id": inst["loan_id"],
            "customer_id": loan["customer_id"], "customer_name": customer_name,
            "number": inst["number"], "total_installments": loan["number_of_installments"],
            "amount": inst["amount"], "updated_amount": round(updated_amount, 2),
            "due_date": inst["due_date"], "status": current_status,
            "paid_at": inst.get("paid_at"), "days_overdue": days_overdue,
            "interest_rate": loan["interest_rate"],
            "loan_amount": loan["amount"], "loan_total": loan["total_amount"]
        })
...
async def get_installments_stats(user: dict = Depends(require_user)):
    ...
    loan_ids = [l["id"] for l in loans]
    loan_map = {l["id"]: l for l in loans}
    insts = await sb_many('installments', 'loan_id, amount, status, due_date', in_={'loan_id': loan_ids})

    today = datetime.now(timezone.utc).date()
    total_pending = total_overdue = total_paid = 0
    pending_amount = overdue_amount = 0
    ...
    return {
        "total_pending": total_pending, "total_overdue": total_overdue, "total_paid": total_paid,
        "pending_amount": round(pending_amount, 2), "overdue_amount": round(overdue_amount, 2)
    }
...
async def get_dashboard(user: dict = Depends(require_user)):
    ...
    if loans:
        loan_ids = [l["id"] for l in loans]
        unpaid = await sb_many('installments', 'updated_amount, amount, due_date', in_={'loan_id': loan_ids}, neq={'status': 'paid'})
        for inst in unpaid:
    ...
    return {
        "total_loaned": round(total_loaned, 2), "total_with_interest": round(total_with_interest, 2),
        "total_received": round(total_received, 2), "total_pending": round(total_pending, 2),
        "customers_count": customers_count, "overdue_count": overdue_count, "loans_count": len(loans)
    }
...
async def get_user_settings(user: dict = Depends(require_user)):
    u = await sb_one('users', 'id, name, email, created_at, default_interest_rate, default_interval_days', eq={'id': user["id"]})
    if not u:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    return {
        "id": u["id"], "name": u["name"], "email": u["email"], "created_at": u.get("created_at"),
        "preferences": {"default_interest_rate": u.get("default_interest_rate"), "default_interval_days": u.get("default_interval_days")}
    }
...
async def change_password(data: PasswordChangeRequest, user: dict = Depends(require_user)):
    ...
    await sb_update('users', {"password_hash": hash_password(data.new_password)}, eq={'id': user["id"]})
    return {"message": "Senha alterada com sucesso"}


@settings_router.get("/preferences")
async def get_preferences(user: dict = Depends(require_user)):
    u = await sb_one('users', 'default_interest_rate, default_interval_days', eq={'id': user["id"]})
    if not u:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    return u
...
async def get_admin_stats(user: dict = Depends(require_admin)):
    ...
    return {
        "total_users": total_users, "total_loans": len(loans),
...
async def list_users(user: dict = Depends(require_admin)):
    return await sb_many('users', 'id, name, email, role, is_blocked, created_at', eq={'role': 'user'})


@admin_router.get("/users/{user_id}")
async def get_user(user_id: str, user: dict = Depends(require_admin)):
    u = await sb_one('users', 'id, name, email, role, is_blocked, created_at', eq={'id': user_id})
    if not u:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    return u
...
async def admin_create_user(data: AdminCreateUser, user: dict = Depends(require_admin)):
    ...
    rows = await sb_insert('users', {
        ...
        "role": "user", "is_blocked": False, "created_at": datetime.now(timezone.utc).isoformat()
    })
    u = rows[0]
    return {"id": u["id"], "name": u["name"], "email": u["email"], "role": u["role"], "is_blocked": u["is_blocked"], "created_at": u["created_at"]}
...
async def admin_update_user(user_id: str, data: AdminUpdateUser, user: dict = Depends(require_admin)):
    ...
    return await sb_one('users', 'id, name, email, role, is_blocked, created_at', eq={'id': user_id})
    ...
    try:
        ...
        if not existing:
            await sb_insert('users', {
                ...
                "name": "Administrador", "role": "admin", "is_blocked": False,
                "created_at": datetime.now(timezone.utc).isoformat()
            })
            logger.info(f"Admin user created: {admin_email}")
        elif not verify_password(admin_password, existing["password_hash"]):
            await sb_update('users', {"password_hash": hash_password(admin_password)}, eq={'id': existing["id"]})
...
async def simulate_loan(data: dict):
    amount = float(data.get("amount"))
    rate = float(data.get("rate"))
    installments = int(data.get("installments"))

    # JUROS SIMPLES (igual seu sistema)
