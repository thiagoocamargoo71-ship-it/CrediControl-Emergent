...
async def list_creditor_customers(creditor_id: str, user: dict = Depends(require_admin)):
    ...
    customers = await sb_many(
        "customers",
        "id, user_id, name, phone, email, document, address, notes, created_at",
        eq={"user_id": creditor_id},
        order="created_at"
    )
...
@admin_router.get("/installments/{installment_id}/details")
async def get_admin_installment_details(installment_id: str, user: dict = Depends(require_admin)):
    ...
    loan = await sb_one(
        "loans",
        "id, user_id, customer_id, amount, total_amount, interest_rate, number_of_installments, created_at",
    ...
    customer = await sb_one(
        "customers",
        "id, name, phone, email, document, address, notes, created_at",
...
@admin_router.get("/installments/overdue")
async def list_admin_overdue_installments(user: dict = Depends(require_admin)):
    loans = await sb_many(
        "loans",
        "id, user_id, customer_id, amount, total_amount, interest_rate, number_of_installments"
    )
    ...
    customer_ids = list(set(loan["customer_id"] for loan in loans))
    creditor_ids = list(set(loan["user_id"] for loan in loans))
    ...
    creditor_map = {creditor["id"]: creditor for creditor in creditors}
    ...
    for inst in installments:
        ...
        customer = customer_map.get(loan["customer_id"], {})
        creditor = creditor_map.get(loan["user_id"], {})

        overdue_rows.append({
            "installment_id": inst["id"],
            "creditor_id": loan["user_id"],
            "creditor_name": creditor.get("name"),
            "creditor_email": creditor.get("email"),
            "customer_id": loan["customer_id"],
